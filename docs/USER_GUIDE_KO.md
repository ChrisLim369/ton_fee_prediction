...
- GitHub Actions에서 수동 실행할 때 최근 데이터, hourly_features.csv, predictions.csv, docs/figures/*.svg, docs/figures/*.png를 갱신합니다.

.github/workflows/daily_model_retrain.yml
- GitHub Actions에서 수동 실행할 때 모델을 재학습하고 model 결과 파일, forecast, chart를 갱신합니다.
...
```text
Netlify Free 팀은 월간 credit 제한이 있습니다.
매시간 main에 commit해서 production deploy를 만들면 Netlify credit을 빠르게 소진하고,
한 프로젝트가 제한을 넘으면 같은 팀의 모든 Netlify 프로젝트가 pause될 수 있습니다.
따라서 현재 GitHub Actions workflow는 자동 schedule이 아니라 수동 실행 전용입니다.
```
...
```text
TONCENTER_API_KEY
NETLIFY_BUILD_HOOK_URL  # 선택 사항. production deploy를 유발하므로 신중하게 사용하세요.
```
