...
### Cloudflare Pages Telegram webhook

Mac 터미널을 계속 켜두지 않고 24시간 운영하려면 Cloudflare Pages Function webhook을 사용합니다.

웹훅 단일 소스는 `functions/telegram-webhook.ts`입니다. `src/telegram_bot.py`는 로컬 polling/진단 전용입니다.
...
functions/telegram-webhook.ts
- Cloudflare Pages에서 동작하는 Telegram webhook 함수입니다.
- Telegram POST update를 받아 dashboard 응답을 보냅니다.

wrangler.jsonc
- Cloudflare Pages 설정입니다.
- output directory는 docs입니다.

package.json / package-lock.json
- Cloudflare Pages Function TypeScript 검증에 필요한 Node dependency와 npm script를 관리합니다.

tsconfig.worker.json
- Cloudflare Pages Function TypeScript 컴파일 설정입니다.

docs/telegram_bot.md
- 로컬 실행, Cloudflare Pages 배포, webhook 등록, 명령어, 보안 주의사항을 정리한 상세 설명서입니다.
...
- GitHub Actions에서 자동 또는 수동 실행될 때 최근 데이터, hourly_features.csv, predictions.csv, docs/figures/*.svg, docs/figures/*.png를 갱신합니다.

.github/workflows/daily_model_retrain.yml
- GitHub Actions에서 자동 또는 수동 실행될 때 모델을 재학습하고 model 결과 파일, forecast, chart를 갱신합니다.
...
Cloudflare Pages > Settings > Variables and Secrets에서 두 값을 Encrypt로 저장합니다.
...
`GET /telegram-webhook`은 health-check JSON을 반환합니다.
...
같은 Telegram bot token으로 로컬 polling 봇과 Cloudflare webhook을 동시에 운영하지 마세요. Cloudflare Pages로 운영할 때는 로컬 `src/telegram_bot.py` 프로세스를 중지하세요.

### 자동 forecast refresh

Telegram 사용자가 `/forecast`를 입력할 때 Telegram handler가 직접 데이터를 수집하거나 모델을 학습하지 않습니다. 대신 GitHub Actions가 백그라운드에서 output 파일을 갱신하고 `docs/data/`에 미러한 뒤 main push로 Cloudflare Pages가 자동 재배포됩니다. Telegram bot은 최신 정적 파일을 읽습니다.
...
-> main push로 Cloudflare Pages 자동 재배포
-> Telegram /forecast가 최신 정적 파일 읽기
```
...
```text
main push는 Cloudflare Pages 자동 재배포를 트리거합니다.
데이터 수집, 모델 학습, chart 생성 같은 무거운 작업은 GitHub Actions에서만 실행합니다.
```
